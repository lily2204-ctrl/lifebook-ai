...
- **Bookpod's API: trust their WordPress plugin, not their PDFs.** `BookPod Author Tools` v2.2.2 on wordpress.org is the code Bookpod write against their own API; their Jan-2025 PDFs describe an upload flow that no longer exists, and following them cost us the first live run. Book creation is 3 steps — ask for signed GCS upload URLs, PUT both PDFs there, then create the record referencing them as `gs://` URIs. `POST /api/v1/books` accepts only one file field, `images`. Detail in LIFEBOOK_SPEC.md §5.
- **`createOrder` is the only call that spends money.** It lives alone in `print-pdf/bookpod-order.cjs` behind `BOOKPOD_ORDERS_ENABLED=true` — leave that env var unset unless a real order is being placed on purpose.
